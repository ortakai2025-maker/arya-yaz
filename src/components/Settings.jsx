                ...
                {weekDays.map((day, dayIdx) => {
                  ...
                  return (
                    <div key={dayIdx} className="bg-white rounded-lg shadow-lg p-6 border-l-4 border-indigo-600">
                      ...
                      <div className="mb-6">
                        <p className="text-sm font-semibold text-slate-700 mb-3">🎯 Şablonlar (Birden Fazla Seçebilirsin):</p>
                        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2">
                          {TEMPLATE_NAMES.map(template => {
                            const templates = dayPlan.templates || [];
                            const isSelected = templates.includes(template);
                            return (
                              <button
                                key={template}
                                onClick={() => {
                                  ...
                                  if (isSelected) {
                                    newTemplates = newTemplates.filter(t => t !== template);
                                  } else {
                                    newTemplates.push(template);
                                  }
                                  weeklyPlan[dateKey] = { ...dayPlan, templates: newTemplates };
                                  updateData({ weeklyPlan });
                                }}
